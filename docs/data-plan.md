...
- 资料页：每个车手或车队并发请求整季正赛、冲刺和排位三份历史，合并后缓存 1 小时；不按每一轮分别请求。
- Widget：只读取 App Group 中的已整理数据，不直接高频请求 API。
- 手动“检查更新”：忽略普通缓存时长，但仍遵守数据源限流。

Jolpica 请求使用可识别的 User-Agent，例如 `Apex/1.0 iOS`。匿名访问当前限制为每秒 4 次、每小时 500 次；收到 429 时优先遵循 `Retry-After`，否则指数退避。
